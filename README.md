...
面向公司同事的批量生产工作流，重做了三大块的交互：

**任务管理**
- 浮动批量操作条：选中任务后粘性显示「已选 N 个 / 全选 / 重试失败 / 删除 / 清除选择」。
- 任务详情弹窗新增桌面端侧边锚点导航，10 个分区一键跳转（基本信息 / 标签 / 自动下载 / Prompt / 增强提示词 / 轮询 / 错误 / 重试历史 / 参考图 / 视频）。
- 「清空任务」改为对话框，支持「全部 / 仅失败+已取消 / 早于 N 天 + 关键字」三种范围，附带前 3 个样例 ID 预览。
- 筛选区拆两行：搜索 + 日期 + 排序 / 平台 + 模式 + 标签；新增「清除全部筛选」一键复位。

**视频生成**
- 「还没有可用平台」空态 hero：一键跳配置 + 内置平台说明。
- 模型缺失时附「控制台 → 模型市场 → 复制 ID」提示 + 一键复制 Base URL。
- 参数区标题旁显示「可见 N 项 / 自动 M 项」徽章，折叠时也不丢失信息。
- 「开始生成」禁用态去强：去阴影 + 灰底灰字 + cursor。
- 词库按钮加 badge 条目数；词库面板从右栏移至 Prompt 卡正下方。
- 侧边栏品牌统一为 `AIVideoForge`，页脚版本与 `package.json` 对齐。

**离线队列**
- 新增错误归类统计面板（按错误消息前 40 字符聚合 + 计数）。
- 每条记录显示 `lastError`，支持 `↑ / ↓` 手动重排提交顺序。
- 新增 `reorderOfflineTask` Context API。

**系统配置**
- 新增二级导航：概览 / 内置平台 / 自定义平台 / 系统设置。
- 「系统设置」重排为 5 张主题卡片（自动下载 / 网络代理 / 上传 Cookie / Cloudreve / 应用更新），每张含状态徽章（已开启 / 未配置 / 就绪 …）。
- 「危险区」独立为红边警示卡，清除敏感凭据操作更醒目。

**横切能力**
- 全局快捷键体系：`Ctrl/⌘+Shift+G/T/,` 跳页、`/` 聚焦当前页主搜索、`?` 打开速查、`Esc` 关闭顶层弹窗；旧的 `Ctrl+F` / `Ctrl+R` 保留并迁移到声明式 `useShortcuts` Hook。
- 侧边栏新增「快捷键」按钮。
- 新手引导 v2：双栏布局（截图 + 步骤），支持 `← / → / Esc` 操作；最后一步会跳转并自动滚到目标页；版本号 1 → 2，已完成 v1 的同事会再次看到。
- 暗色主题补漏：浮动操作条 / 详情侧边导航 / 新手引导左侧 / `kbd` 元素。

### v1.2.0

新增 Cloudreve 存储桶上传、Cloudreve 实例可配置化、MD5 移入 Worker、修复素材缩略图损坏，落实安全 / 正确性 / 性能加固。
